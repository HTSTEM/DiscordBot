...
class Misc:
    @commands.command()
    async def roll(self, ctx, sides: int, number: int = 1):
        '''Rolls a dice.'''
        if sides < 1 or number < 1:
        ...
        if sides == 1:
            return await ctx.send('All {} of the 1-sided dice shocking rolled a 1.'.format(number))
        ...
        if sides == 666:
            ...
            return await ctx.send('{}? That\'s how many users are on the server! Well, your die rolled a {}, '
                                  'and according to the cache, that member is `{}`.'.format(sides, rolled + 1,
                                                                                            chosen.name))
        result = ', '.join(str(random.randint(1, sides)) for i in range(number))
        await ctx.send('The {} {}-sided {} rolled {}.'.format(number, sides, 'die' if number == 1 else 'dice', result))
